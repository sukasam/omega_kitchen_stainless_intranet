...
if ($_POST["mode"] <> "") {
	...
	$a_sdate = explode("/", $_POST['stock_approve_date']);
	$_POST['stock_approve_date'] = $a_sdate[2] . "-" . $a_sdate[1] . "-" . $a_sdate[0];
	...
	if ($_POST["mode"] == "add") {
		...
		for ($i = 0; $i <= count($_POST['cpro']); $i++) {
			if ($_POST['cpro'][$i] != "") {
				...
				@mysqli_query($conn, "INSERT INTO `s_group_stock_order_bill_pro` (`id`, `id_bill`, `sparpart_id`, `sparpart_qty`, `sparpart_unit_price`, `sparpart_total`) VALUES (NULL,'" . $id . "','" . $_POST['cpro'][$i] . "','" . $_POST['camount'][$i] . "','" . $_POST['cprice'][$i] . "','" . $cost . "');");

				@mysqli_query($conn, "UPDATE `s_group_stock_project` SET `group_stock` = `group_stock`+'" . $_POST['camount'][$i] . "' WHERE `group_id` = '" . $_POST['cpro'][$i] . "';");
			}
		}
		...
		$mpdf->Output('../../upload/stockin_order/' . $chaf . '.pdf', 'F');
	...
	if ($_POST["mode"] == "update") {
		...
		$sql2 = "select * from s_group_stock_order_bill_pro where id_bill = '" . $_REQUEST[$PK_field] . "'";
		$quPro = @mysqli_query($conn, $sql2);
		while ($rowPro = mysqli_fetch_array($quPro)) {
			@mysqli_query($conn, "UPDATE `s_group_stock_project` SET `group_stock` = `group_stock`-'" . $rowPro['sparpart_qty'] . "' WHERE `group_id` = '" . $rowPro['sparpart_id'] . "';");
		}

		@mysqli_query($conn, "DELETE FROM `s_group_stock_order_bill_pro` WHERE `id_bill` = '" . $_REQUEST[$PK_field] . "'");
...
if ($_GET["mode"] == "update") {
	...
	$qcInfo = get_qc_product($conn,$qc_id);
	$pro_listS = explode(',',$qcInfo['pro_list']);

	$a_sdate = explode("-", $stock_approve_date);
	$stock_approve_date = $a_sdate[2] . "/" . $a_sdate[1] . "/" . $a_sdate[0];
								...
								<fieldset>
											...
											<tr>
												...
												<td>
													<table class="formFields" cellspacing="0" width="100%">
														...
														<tr>
															<td nowrap class="name">เลขที่ใบตรวจสินค้า</td>
															<td nowrap class="name">
																<span id="qc_number"><?php echo get_qc_product_number($conn, $qc_id); ?></span>
																<input type="hidden" id="qc_id" name="qc_id" value="<?php echo $qc_id; ?>">
															</td>
														</tr>
														<tr>
									...
									<!-- <table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:12px;text-align:center;">
										<tr>
											...
											<td width="11%" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding:5px;text-align:center;display:none;"><strong>ราคา / ต่อหน่วย</strong></td>
											<td width="11%" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding:5px;text-align:center;display:none;"><strong>จำนวนเงิน</strong></td>
										</tr>
										<tbody id="exp" name="exp">
											<?php
											$sub_id = $_GET['sub_id'];
											$quQry = mysqli_query($conn, "SELECT * FROM `s_group_stock_order_bill_pro` WHERE id_bill = '" . $sub_id . "' ORDER BY id ASC");
											...
											?>
												<tr>
													...
													<td style="border:1px solid #000000;padding:5;text-align:center;display:none;">
													...
													<td style="border:1px solid #000000;padding:5;text-align:right;display:none;" id="ctotal<?php echo $rowCal; ?>">
											...
											?>
													<tr>
														...
														<td style="border:1px solid #000000;padding:5;text-align:center;">
															...
															<input type="hidden" name="cpro[]" value="<?php echo $rowPro['cpro'];?>">
														</td>
														<td style="border:1px solid #000000;text-align:left;padding:5;">
														...
														<td style="border:1px solid #000000;padding:5;text-align:center;">
															...
															<input type="hidden" name="camount[]" value="<?php echo $rowPro['camount']; ?>">
														</td>
													</tr>
											<?php
												}
									...
									<script>
										...
										function addExp() {
											...
											filedMore += '	<td style="border:1px solid #000000;padding:5;text-align:center;display:none;">';
											...
											filedMore += '	<td style="border:1px solid #000000;padding:5;text-align:right;display:none;" id="ctotal' + countBox + '">';
											filedMore += '	0</td>';
											filedMore += '</tr>';
													...
													<tr>
														<td style="border-bottom:1px solid #000000;padding-bottom:10px;font-size:12px;font-family:Verdana, Geneva, sans-serif;text-align:center;"><strong>
																<select name="stock_admin" id="stock_admin" class="inputselect" style="width:50%;">
																	...
																	?>
																		<option value="<?php echo $row_custec['group_id']; ?>" <?php if ($row_custec['group_id'] == $stock_admin) {
																																	echo 'selected';
											...
											<td width="50%" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;text-align:center;padding-top:10px;padding-bottom:10px;">
												<table width="100%" border="0" cellspacing="0" cellpadding="0">
													<tr>
														...
														if($stock_approve != ''){
														?>
															<?php echo getsalename($conn,$stock_approve);?>
														<?php
														}else{
															echo "<br>";
														}
														?>
														<input type="hidden" name="stock_approve" value="<?php echo $stock_approve;?>">  
														</td>
													</tr>
													<tr>
