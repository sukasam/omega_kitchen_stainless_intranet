	...
	if ($_POST["mode"] <> "") { 
			...
			$_POST['stock_approve_date'] = '0000-00-00';
				...
				for($i=0;$i<=count($_POST['cpro']);$i++){
					if($_POST['cpro'][$i] != ""){
						...
						@mysqli_query($conn,"UPDATE `s_group_stock_order` SET `group_stock` = `group_stock`+'".$_POST['camount'][$i]."' WHERE `group_id` = '".$_POST['cpro'][$i]."';");
					}
				}
		...
		if ($_POST["mode"] == "update" ) { 
				...
				while($rowPro = mysqli_fetch_array($quPro)){
					@mysqli_query($conn,"UPDATE `s_group_stock_order` SET `group_stock` = `group_stock`-'".$rowPro['sparpart_qty']."' WHERE `group_id` = '".$rowPro['sparpart_id']."';");
				}
