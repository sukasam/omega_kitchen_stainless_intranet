<?php    
	...
	if($_GET['action'] == 'getsparkey'){
		...
		echo "SELECT group_name FROM s_group_stock_order ".$consd." ORDER BY group_name ASC";
		$qu_cus = mysqli_query($conn,"SELECT * FROM s_group_stock_order ".$consd." ORDER BY group_name ASC");
		while($row_cus = @mysqli_fetch_array($qu_cus)){
			?>
			 <tr>
	...
	if($_GET['action'] == 'getspar'){
		...
		$qupros1 = @mysqli_query($conn,"SELECT * FROM s_group_stock_order ORDER BY group_name ASC");
		while($row_qupros1 = @mysqli_fetch_array($qupros1)){
		  ?>
