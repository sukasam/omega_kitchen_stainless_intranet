...
function checkTotal(key){
	...
	var camount = parseFloat($("#camount"+key).val().replace(/,/g, ''));
	var cprice = parseFloat($("#cprice"+key).val().replace(/,/g, ''));
	var total = 0.00;
	...
	if(camount == "" || camount == 0){
		$("#ctotal"+key).html(total.toFixed(2));
	}
	...
	if(cprice != 0 && camount != 0){
		total = parseFloat(camount*cprice);
		$("#ctotal"+key).html(total.toFixed(2));
	}
	...
	for(var i=1;i<rowCal;i++){
		if($("#camount"+i).val() != "" && $("#cprice"+i).val() != ""){
			sumTotal = parseFloat(sumTotal + ($("#camount"+i).val().replace(/,/g, '')*$("#cprice"+i).val().replace(/,/g, '')));
		}
	}
...
$qupro1 = @mysqli_query($conn, "SELECT * FROM s_group_sparpart ORDER BY group_name ASC");
        ...
        ?>
				  ...
				  <td style="border:1px solid #000000;padding:5;text-align:center;">
					<input type="text" name="camount[]" value="<?php echo number_format($rowPro['camount'], 2); ?>" id="camount<?php echo $rowCal; ?>" class="inpfoder" style="width:100%;text-align:center;" onkeypress="return isNumberKey(event)" onblur="checkTotal('<?php echo $rowCal; ?>')">

				  </td>
				  <td style="border:1px solid #000000;padding:5;text-align:center;">
					<input type="text" name="cprice[]" value="<?php echo number_format($rowPro['cprice'], 2); ?>" id="cprice<?php echo $rowCal; ?>" class="inpfoder" style="width:100%;text-align:center;" onkeypress="return isNumberKey(event)" onblur="checkTotal('<?php echo $rowCal; ?>')">
				  </td>
				  <td style="border:1px solid #000000;padding:5;text-align:right;" id="ctotal<?php echo $rowCal; ?>">
				  	<?php echo number_format($rowPro['camount'] * $rowPro['cprice'], 2) ?>
				  </td>
				</tr>
				<?php
}
...
$qupro1 = @mysqli_query($conn, "SELECT * FROM s_group_sparpart ORDER BY group_name ASC");
        ...
        ?>
				  ...
				  <td style="border:1px solid #000000;padding:5;text-align:center;">
					<input type="text" name="camount[]" value="<?php echo number_format($rowPro['sparpart_qty'], 2); ?>" id="camount<?php echo $rowCal; ?>" class="inpfoder" style="width:100%;text-align:center;" onkeypress="return isNumberKey(event)" onblur="checkTotal('<?php echo $rowCal; ?>')">
				  </td>
				  <td style="border:1px solid #000000;padding:5;text-align:center;">
