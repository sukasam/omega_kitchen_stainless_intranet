...
<title>ตารางงานเปิดใบสั่งผลิตสินค้า (วันที่ <?php  echo sprintf("%02d",$_GET['day']).'-'.sprintf("%02d",$_GET['month']).'-'.sprintf("%02d",($_GET['year']+543));?> )</title>
...
<table width="100%" border="0" class="tbservice">
  ...
  <?php  
	...
	$qu_service = @mysqli_query($conn,"SELECT * FROM s_order_product WHERE job_open = '".$con."' AND st_setting = 0");
	$romn = 1;
	...
	$scstatus = "<a href=\"../../upload/order_product/".$chaf.".pdf\" target=\"_blank\" style=\"text-decoration: none;\"><span style=\"color:".$rowColor.";\">".$row_serv['sv_id']."</span></a>";
