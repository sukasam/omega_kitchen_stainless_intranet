<?php    
	...
	if($_GET["action"] == "delete"){
		...
		if ($code == "1") {
			...
			while($rowPro = mysqli_fetch_array($quPro)){
				@mysqli_query($conn,"UPDATE `s_group_stock_order` SET `group_stock` = `group_stock`+'".$rowPro['opens']."' WHERE `group_id` = '".$rowPro['lists']."';");
			}
	...
	//-------------------------------------------------------------------------------------
	 ...
	 if ($_GET[cc] != "" and $_GET[tt] != "") { 
		...
		@mysqli_query($conn,$sql_status);
		/*$sql_fostatus = "update s_fopj set status = '".$status."' where fo_id = '$_GET[cus_id]'";
...
<UL class=shortcut-buttons-set>
  <LI><A class=shortcut-button href="update.php?mode=add<?php     if ($param <> "") echo "&".$param; ?>"><SPAN><IMG  alt=icon src="../images/pencil_48.png"><BR>
    เพิ่ม</SPAN></A></LI>
	<LI><A class=shortcut-button href="../bill_shopping_approve/"><SPAN><IMG  alt=icon src="../images/icons/icon-48-user.png" width="48"><BR>
      ...
      <TBODY>
        <?php     
					...
					if ($sortby =="") $sortby ="DESC";
				   	...
				   	$sql = "SELECT sr . * , fd.cd_name FROM $tbl_name AS sr, s_fopj AS fd WHERE sr.cus_id = fd.fo_id";
