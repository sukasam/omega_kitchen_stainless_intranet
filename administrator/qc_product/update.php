...
if ($_POST["mode"] != "") {
    ...
    if ($_POST['chkproarray'] >= 1) {
        for ($i = 1; $i <= $_POST['chkproarray']; $i++) {
            $listChkPro .= $_POST['chkpasspro' . $i] . ",";
        }
        $listChkPro = substr($listChkPro, 0, -1);
    }
...
if ($_GET["mode"] == "update") {
    ...
    $checklistpro = explode(',', $chklistpro);
...
<input type="text" name="sv_id" value="<?php if ($sv_id == "") {echo check_product_order($conn);} else {echo $sv_id;}
...
if ($_GET['mode'] != 'add') {
    ...
    <tr>
	  ...
	  <td width="10%" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding:5px;text-align:center;"><strong>การตรวจสอบ</strong></td>
    ...
    if (in_array($rowCal, $pro_listS)) {
        ...
        ?>
			<tr>
			  ...
			  <td style="border:1px solid #000000;padding:5;text-align:center;">
			  <input type="radio" name="chkpasspro<?php echo $rowCalnum; ?>" value="1" <?php if ($checklistpro[$rowCalnum - 1] == "1") {echo 'checked';}?>><strong>ผ่าน</strong> | <input type="radio" name="chkpasspro<?php echo $rowCalnum; ?>" value="0" <?php if ($checklistpro[$rowCalnum - 1] == "0" || $checklistpro[$rowCalnum - 1] == "") {echo 'checked';}?>><strong>ไม่ผ่าน</strong>
			  </td>
			</tr>
			<?php
}
...
?>
	...
	<input type="hidden" name="chkproarray" value="<?php echo $rowCalnum; ?>">
