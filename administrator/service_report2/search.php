    ...
    function get_customer(fo_id, cname, chk, sr_id) {
        ...
        xmlHttp.onreadystatechange = function() {
            if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
                ...
                self.opener.document.getElementById("rsnameid").innerHTML =
                    "<input type=\"hidden\" name=\"cus_id\" value=\"" + fo_id + "\">";
        ...
        <tr>
            <td colspan="2">
                <form action="search.php" method="post" name="form1">
                    ...
                    <!-- <input type="radio" name="cusSource" value="fo" <?php if ($_GET['chk'] == "fo" || !isset($_GET['chk'])) {
    echo 'checked';
}?> Onclick="checkVal(this)"> First Order&nbsp;&nbsp;&nbsp;
	  <input type="radio" name="cusSource" value="po" <?php if ($_GET['chk'] == "po") {
    echo 'checked';
}?> Onclick="checkVal(this)"> Project Order
	  <input type="radio" name="cusSource" value="fopj" <?php if ($_GET['chk'] == "fopj") {
    echo 'checked';
}?> Onclick="checkVal(this)"> FO/PJ -->
                </form>
            </td>
        </tr>
        <tr>
            <td colspan="2"><strong>ค้นหา&nbsp;&nbsp;:&nbsp;&nbsp;</strong>
                <input type="text" name="textfield" id="textfield" style="width:85%;"
                    onkeyup="get_cusorprod(this.value,'<?php echo 'fopj'; ?>');" />
            </td>
        </tr>
    </table>
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tv_search">
...
$qu_cus = mysqli_query($conn, "SELECT * FROM " . $tableDB . " WHERE 1 AND st_setting = '0' ORDER BY sr_id DESC");
    ...
    ?>
        <tr>
            <td><A href="javascript:void(0);"
                    onclick="get_customer('<?php echo $fopjInfo['fo_id']; ?>','<?php echo $fopjInfo['cd_name']; ?>','<?php echo 'fopj'; ?>','<?php echo $row_cus['sr_id']; ?>');"><?php echo $row_cus['sv_id'] . " | " . $fopjInfo['cd_name']; ?>
                    (<?php echo $fopjInfo['loc_name'] ?>)</A></td>
        </tr>
        <?php
}
?>
