<?php    
	...
	if ($_POST["mode"] <> "") { 
		...
		if ($_POST["mode"] == "add") { 
			...
			$_POST['stock_approve_date'] = '0000-00-00';
				...
				for($i=0;$i<=count($_POST['cpro']);$i++){
					if($_POST['cpro'][$i] != ""){
						...
						@mysqli_query($conn,"INSERT INTO `s_group_stock_project_bill_pro` (`id`, `id_bill`, `sparpart_id`, `sparpart_qty`, `sparpart_unit_price`, `sparpart_total`) VALUES (NULL,'".$id."','".$_POST['cpro'][$i]."','".$_POST['camount'][$i]."','".$_POST['cprice'][$i]."','".$cost."');");
			...
			$mpdf->Output('../../upload/stockin_project/'.$chaf.'.pdf','F');
		...
		if ($_POST["mode"] == "update" ) { 
				...
				$sql2 = "select * from s_group_stock_project_bill_pro where id_bill = '".$_REQUEST[$PK_field]."'";
				...
				@mysqli_query($conn,"DELETE FROM `s_group_stock_project_bill_pro` WHERE `id_bill` = '".$_REQUEST[$PK_field]."'");
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:12px;text-align:center;">
    ...
    <?php    
		$sub_id = $_GET['sub_id'];
		$quQry = mysqli_query($conn,"SELECT * FROM `s_group_stock_project_bill_pro` WHERE id_bill = '".$sub_id."' ORDER BY id ASC");
