<?php    
	...
	if($_GET['action'] == 'getprojectL'){
		...
		//echo "SELECT * FROM s_group_stock_project ".$consd." ORDER BY group_name ASC";
		$qu_cus = mysqli_query($conn,"SELECT * FROM s_group_stock_project ".$consd." ORDER BY group_name ASC");
		while($row_cus = @mysqli_fetch_array($qu_cus)){
			?>
			 <tr>
				<td><A href="javascript:void(0);" onclick="get_product('<?php echo $row_cus['group_id'];?>','<?php echo $row_cus['group_name'];?>','<?php echo $keys;?>','<?php echo $row_cus['group_spar_id'];?>','<?php echo $col;?>','<?php echo $row_cus['group_size'];?>','<?php echo $row_cus['group_sn'];?>','<?php echo $row_cus['group_price'];?>');"><?php echo $row_cus['group_spar_id']." | ".$row_cus['group_name']." | ".$row_cus['group_size'];?></A></td>
			  </tr>
			<?php    	
		}
		//echo "SELECT cd_name FROM s_first_order ".$consd." ORDER BY cd_name ASC";
